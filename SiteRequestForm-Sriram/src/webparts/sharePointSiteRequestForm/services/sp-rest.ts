...
function getListItemEntityTypeName(siteUrl: string, spHttpClient: SPHttpClient, listName: string): Promise<SPHttpClientResponse> {
  ...
  /*function populateLifeExpectancy(): Array<any> {
      var items: string[];
      sp.web.lists.getByTitle('Test').items.select('Title').get()
      .then((testitems)=>{
          testitems.map((testitem)=>items.push(testitem.Title)
          )        
      })       
      return items;
  }*/
